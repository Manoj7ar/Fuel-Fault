 ...
 */
function handleMockApi(req, res, url, bodyStr) {
...
window.__fflBundledDemoResolve = function (absoluteUrl, method, bodyStr) {
  var url = new URL(absoluteUrl);
  var req = { method: method || 'GET' };
  var status = 200;
  ...
  try {
    ...
    body = JSON.stringify({ detail: String(e && e.message ? e.message : e) });
  }
  return { status: status, headers: headers, body: body };
};

(function () {
  if (typeof window === 'undefined' || !window.__FFL_BUNDLED_DEMO || typeof window.fetch !== 'function') return;
  ...
  window.fetch = function (input, init) {
    ...
    try {
      ...
      if (u.pathname === '/mock-api' || u.pathname.indexOf('/mock-api/') === 0) {
        var out = window.__fflBundledDemoResolve(u.href, method, bodyStr);
        var hdrs = out.headers || { 'Content-Type': 'application/json; charset=utf-8' };
